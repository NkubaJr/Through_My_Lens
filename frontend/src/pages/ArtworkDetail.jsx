...
export default function ArtworkDetail() {
 ...
 const handleLike = async () => {
  ...
  try {
    ...
    const res = await axios.post(`http://localhost:5000/api/artworks/${id}/like`, {
  ...
  const handleComment = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/artworks/${id}/comment`, {
        name: user.username,
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px 24px 60px' }}>
      ...
      <div style={{
        ...
        overflow: 'hidden', boxShadow: '0 4px 24px rgba(59,31,14,0.1)'
      }}>
        <img
        ...
        <div style={{ padding: '32px' }}>
          ...
          <div style={{ borderTop: '1px solid #f0d9b5', paddingTop: '24px', marginTop: '8px' }}>
            <h3 style={{
              fontFamily: 'Playfair Display, serif',
              fontSize: '1.1rem', color: '#3b1f0e', marginBottom: '12px'
            }}>
              The Story
            </h3>
            <p style={{ color: '#5c3212', lineHeight: '1.8', fontSize: '0.95rem' }}>
              {artwork.story}
            </p>
          </div>
          ...
          <div style={{
            ...
            {/* Connect — not shown to artwork owner */}
          ...
          {showCommentForm && user && (
            <div style={{
              ...
              backgroundColor: '#f5ebe0', borderRadius: '16px'
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                <div style={{
                  ...
                  fontSize: '0.75rem', fontWeight: '700', color: '#3b1f0e'
                }}>
                  {user.username.charAt(0).toUpperCase()}
                </div>
                <p style={{ fontWeight: '600', color: '#3b1f0e', fontSize: '0.88rem' }}>
                  {user.username}
                </p>
              </div>
              <textarea
          ...
          {comments.length > 0 && (
            <div style={{ marginTop: '24px' }}>
              <h4 style={{
                fontFamily: 'Playfair Display, serif',
                color: '#3b1f0e', marginBottom: '14px', fontSize: '1rem'
              }}>
                Comments
              </h4>
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{
                          ...
                          backgroundColor: '#e49a3a',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontSize: '0.7rem', fontWeight: '700', color: '#3b1f0e'
                        }}>
                      ...
                      <p style={{ fontSize: '0.75rem', color: '#c9a06a' }}>
                        {new Date(comment.created_at).toLocaleDateString()}
                      </p>
                    </div>
                    <p style={{ color: '#5c3212', fontSize: '0.88rem', lineHeight: '1.6', paddingLeft: '36px' }}>
                      {comment.message}
                    </p>
                  </div>
                ))}
      ...
      {showModal && (
        <ConnectModal artwork={artwork} onClose={() => setShowModal(false)} />
      )}
    </div>
  );
}